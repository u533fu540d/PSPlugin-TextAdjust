# 批量调整工具（Photoshop CEP 版）

一个 **Photoshop CEP 扩展**：按字符属性批量筛选并修改文本图层，也可按尺寸/主色调筛选图片图层并批量缩放。CEP 面板是**非模态**的，面板打开时仍可正常操作 Photoshop。
...
安装后**完全退出并重启 Photoshop**，菜单 `窗口 > 扩展(旧版) > 批量调整工具`。
...
- 图层列表：整行点击或拖选、行高滑杆（过窄时自动隐藏正文预览）、一键 `全选 / 反选 / 清空 / 选中`。
- **选中**：把列表中的选中项同步多选到 Photoshop 图层面板。
- 批量修改：字体系列 + 样式、字号（固定/按比例）、颜色、粗斜、下划线、删除线、大小写、基线、字距、行距、缩放、消除锯齿、自动字距等。
- 图片模块：尺寸与主色调筛选、主色容差、按目标尺寸等比缩放或按百分比缩放、九宫格选择缩放锚点。
- 文本/图片模块底部均提供 `撤销`，用于回退上一轮批量修改。
- 调试输出：记录每次操作的过程与错误。
